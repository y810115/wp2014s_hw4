...
FB.getLoginStatus(function(response) {
  ...
  if (response.status === 'connected') {
    //呼叫api把圖片放到#preview IMG tag 內 ：FB.api
    FB.api('/me/picture', function(response) {
      //console.log(response.data.url);
      var placeholder = document.createElement("div"); 
      $("#main").append(placeholder); //divID.append(preview)
...
//以下為canvas的程式碼，基本上不需多動，依據comments修改即可
    ...
    img.src = "../img/overlay.jpg"; //圖像路徑（路徑自己設，且自己加入想要的圖層）
	var img2 = new Image(); //新增圖像2
	img2.src = "../img/overlayback.jpg" //圖像路徑
